...
func (m *Task) Create() error {
	return db.MasterClient.Model(&m).Create(&m).Error
}

func (m *Task) FindByID(id int64) (info Task, err error) {
	err = db.MasterClient.Model(&m).Where("id = ?", id).First(&info).Error
	return
}

func (m *Task) UpdateStatusByID(id int64, status TaskStatus) error {
	if status == TaskStatusConsult {
		return db.MasterClient.Model(&m).Where("id = ?", id).UpdateColumns(map[string]interface{}{
			"status":     status,
			"start_time": time.Now(),
		}).Error
	} else {
		return db.MasterClient.Model(&m).Where("id = ?", id).UpdateColumn("status", status).Error
	}
}
...
func (m *Task) UpdateDownloadUrlByID(id int64, url string) error {
	return db.MasterClient.Model(&m).Where("id = ?", id).UpdateColumn("download_url", url).Error
}

func (m *Task) UpdateErrLogUrlByID(id int64, url string) error {
	return db.MasterClient.Model(&m).Where("id = ?", id).UpdateColumn("err_log_url", url).Error
}

func (m *Task) PageList(page, size int) (list []Task, count int64, err error) {
