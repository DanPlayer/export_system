...
import (
	"bytes"
	"crypto/md5"
	"export_system/internal/sonyflake"
	"fmt"
	"log"
	"math/rand"
	"reflect"
...
// GetUid 获取唯一ID
func GetUid() string {
	outOrderSnUint, err := sonyflake.Client.NextID()
	if err != nil {
		log.Println(err)
		return ""
	}
	return strconv.FormatUint(outOrderSnUint, 10)
}
